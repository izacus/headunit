...
static int iusb_deinit () {                                              // !!!! Need to better reset and wait a while to kill transfers in progress and auto-restart properly
...
static int switch_device_to_oap_mode() {
  ...
  // close it.
  if (best_device) libusb_ref_device(best_device);
